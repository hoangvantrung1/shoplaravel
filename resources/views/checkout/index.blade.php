                ...
                <form action="{{ route('checkout.store') }}" method="POST" id="checkout-form">
                    ...
                    <div class="bg-white rounded-lg shadow mb-6 overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @if(isset($addresses) && $addresses->count())
                                    <div class="md:col-span-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Chọn địa chỉ đã
                                            lưu</label>
                                        <select id="saved-address" class="w-full px-4 py-2 border rounded-lg">
                                            <option value="">-- Chọn --</option>
                                            @foreach($addresses as $addr)
                                                <option value="{{ $addr->id }}" data-name="{{ $addr->name }}"
                                                    data-phone="{{ $addr->phone }}"
                                                    data-email="{{ $addr->email ?? $addr->email }}"
                                                    data-address="{{ $addr->address_line }}">
                                                    {{ $addr->name }} - {{ $addr->phone }} | {{ $addr->address_line }}
                                                    {{ $addr->is_default ? '(Mặc định)' : '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        <p class="text-xs text-gray-500 mt-1">Chọn sẽ tự động điền thông tin bên dưới.</p>
                                    </div>
                                @endif
                                <div>
                                    <label for="customer_name" class="block text-sm font-medium text-gray-700 mb-1">Họ
                                        và tên *</label>
                                ...
                                <div>
                                    <label for="customer_phone" class="block text-sm font-medium text-gray-700 mb-1">Số
                                        điện thoại *</label>
                            ...
                            <div class="mt-4">
                                <label for="customer_address" class="block text-sm font-medium text-gray-700 mb-1">Địa
                                    chỉ giao hàng *</label>
                    ...
                    <div class="bg-white rounded-lg shadow mb-6 overflow-hidden">
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 gap-4">
                                <div class="flex items-center p-4 border rounded-lg cursor-pointer payment-method"
                                    data-method="cod">
                            ...
                            <div class="border-t border-gray-200 mt-6 pt-6">
                                ...
                                @if(session('coupon'))
                                    <div class="flex justify-between mb-2 text-green-600">
                                        ...
                                        <span>-{{ number_format(session('coupon.discount'), 0, ',', '.') }}₫</span>
                                    </div>
                                @endif
                    ...
                    <div class="p-5">
                        <div class="mb-4">
                            ...
                            <div class="flex justify-between mt-3 pt-3 border-t border-gray-200">
                                ...
                                <span
                                    class="text-lg font-bold text-blue-600">{{ number_format($grandTotal, 0, ',', '.') }}₫</span>
                            </div>
                        </div>
