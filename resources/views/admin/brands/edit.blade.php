...
@section('content')
<h1 class="text-2xl font-bold mb-6">Sửa thương hiệu</h1>
...
<form action="{{ route('admin.brands.update', $brand->id) }}" method="POST" class="bg-white p-6 rounded shadow max-w-xl">
    @csrf
    @method('PUT')
    <div class="mb-4">
        <label class="block text-sm font-medium mb-1">Tên thương hiệu</label>
        <input type="text" name="name" value="{{ old('name', $brand->name) }}" class="w-full border rounded px-3 py-2" required>
    </div>
