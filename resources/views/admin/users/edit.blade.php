...
@section('content')
    <div class="p-6 bg-white rounded shadow max-w-lg mx-auto">
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="mb-4">
                <label class="block mb-1 font-semibold">Active</label>
                <select name="is_active" class="w-full border p-2 rounded">
                    <option value="1" {{ old('is_active', $user->is_active) == 1 ? 'selected' : '' }}>Bật</option>
                    <option value="0" {{ old('is_active', $user->is_active) == 0 ? 'selected' : '' }}>Tắt</option>
                </select>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-yellow-500 text-white px-4 py-2 rounded hover:bg-yellow-600">Cập
                    nhật</button>
            </div>
        </form>
    </div>
