...
@section('content')
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.products.create') }}"
            class="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded shadow transition">Thêm mới</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-green-100">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Tên sản phẩm
                    </th>
                    ...
                    <tr class="hover:bg-green-50 transition">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-center space-x-2 flex justify-center">
                            <a href="{{ route('admin.products.edit', $product->id) }}"
                                class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded shadow transition">Sửa</a>
                            <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST"
                                class="inline-block" onsubmit="return confirm('Bạn có chắc muốn xóa?');">
                                @csrf
                                @method('DELETE')
                                <button
                                    class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded shadow transition">Xóa</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
